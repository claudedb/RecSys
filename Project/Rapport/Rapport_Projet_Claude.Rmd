...
Pour le modèle de comparaison terme-terme, nous faisons simplement une comparaison des cours entre eux avec une corrélation de spearman. Ensuite, on détermine les 10 cours les plus similaires pour chacun des cours. Avec l'entrée d'un cours, on choisit donc la ligne correspondante à ce cours. On utilise le code suivant pour générer les recommendations:
```{r  echo = T}
correlation.termes=cor(t(as.matrix(m.poly)),method="spearman")
correlation.termes[is.na(correlation.termes)]=-1
neighbors.termes <-t(apply(correlation.termes,1,max.nindex.corr))
...
```
###2) TF-IDF
Pour le modèle TF-IDF, on utilise la même logique mais en utilisant une matrice modifiée TF-IDF pour faire les corrélations. Voici le code pour générer les recommandations:
...
recommandations.tfidf <- colnames(m.poly)[neighbors.tfidf[id.cours1,]]
recommandations.tfidf

```
###3) Log-Entropy
Log-Entropy reprend la même logique que le modèle précédent mais avec une transformation log-entropy. Voici le code:  
```{r  echo = T}
pij <- m.poly / rowSums(m.poly)
log2.pij <- log2(pij)
log2.pij[is.infinite(log2.pij)] <- 0
...
recommandations.ent <- colnames(m.poly)[neighbors.ent[id.cours1,]]
recommandations.ent
```
###4) LSA 
Pour les modèles LSA, nous changerons simplement les matrices d'entrée par les matrices avec les différentes transformations. Nous utiliseront le package LSA de R. On utilise la fonction LSA() qui décompose en facteur latent et la fonction as.textmatrix() qui prend les facteurs latents en entrée et retourne une matrice qui sera utilisé pour faire les corrélations.  
  
Voici le code pour LSA avec la matrice d'entrée terme-document originale:  
``` {r  echo = T}
...
recommandations.lsa <- colnames(m.poly)[neighbors.lsa[id.cours1,]]
recommandations.lsa
```
###5) TF-IDF -> LSA
...
recommandations.lsa.tfidf <- colnames(m.poly)[neighbors.lsa.tfidf[id.cours1,]]
recommandations.lsa.tfidf
```
###6) Log-Entropy -> LSA
...
recommandations.lsa.ent<- colnames(m.poly)[neighbors.lsa.ent[id.cours1,]]
recommandations.lsa.ent
